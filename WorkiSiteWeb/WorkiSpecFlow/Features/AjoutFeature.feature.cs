// ------------------------------------------------------------------------------
//  <auto-generated>
//      This code was generated by SpecFlow (http://www.specflow.org/).
//      SpecFlow Version:1.6.1.0
//      SpecFlow Generator Version:1.6.0.0
//      Runtime Version:4.0.30319.237
// 
//      Changes to this file may cause incorrect behavior and will be lost if
//      the code is regenerated.
//  </auto-generated>
// ------------------------------------------------------------------------------
#region Designer generated code
namespace Worki.SpecFlow.Features
{
    using TechTalk.SpecFlow;
    
    
    [System.CodeDom.Compiler.GeneratedCodeAttribute("TechTalk.SpecFlow", "1.6.1.0")]
    [System.Runtime.CompilerServices.CompilerGeneratedAttribute()]
    [NUnit.Framework.TestFixtureAttribute()]
    [NUnit.Framework.DescriptionAttribute("Ajout")]
    public partial class AjoutFeature
    {
        
        private static TechTalk.SpecFlow.ITestRunner testRunner;
        
#line 1 "AjoutFeature.feature"
#line hidden
        
        [NUnit.Framework.TestFixtureSetUpAttribute()]
        public virtual void FeatureSetup()
        {
            testRunner = TechTalk.SpecFlow.TestRunnerManager.GetTestRunner();
            TechTalk.SpecFlow.FeatureInfo featureInfo = new TechTalk.SpecFlow.FeatureInfo(new System.Globalization.CultureInfo("en-US"), "Ajout", "In order to search a working place\r\nAs a user\r\nI want to be able to view the list" +
                    " of localisations", GenerationTargetLanguage.CSharp, ((string[])(null)));
            testRunner.OnFeatureStart(featureInfo);
        }
        
        [NUnit.Framework.TestFixtureTearDownAttribute()]
        public virtual void FeatureTearDown()
        {
            testRunner.OnFeatureEnd();
            testRunner = null;
        }
        
        public virtual void ScenarioSetup(TechTalk.SpecFlow.ScenarioInfo scenarioInfo)
        {
            testRunner.OnScenarioStart(scenarioInfo);
        }
        
        [NUnit.Framework.TearDownAttribute()]
        public virtual void ScenarioTearDown()
        {
            testRunner.OnScenarioEnd();
        }
        
        [NUnit.Framework.TestAttribute()]
        [NUnit.Framework.DescriptionAttribute("Ajout Erreur")]
        [NUnit.Framework.CategoryAttribute("Ajout")]
        public virtual void AjoutErreur()
        {
            TechTalk.SpecFlow.ScenarioInfo scenarioInfo = new TechTalk.SpecFlow.ScenarioInfo("Ajout Erreur", new string[] {
                        "Ajout"});
#line 7
this.ScenarioSetup(scenarioInfo);
#line 8
 testRunner.Given("Je vais dans la page d\'ajout");
#line 9
 testRunner.When("Je clique sur Envoyer");
#line 10
 testRunner.Then("Il doit y avoir des messages d\'erreur");
#line hidden
            testRunner.CollectScenarioErrors();
        }
        
        [NUnit.Framework.TestAttribute()]
        [NUnit.Framework.DescriptionAttribute("Ajout mauvais pattern")]
        [NUnit.Framework.CategoryAttribute("Ajout")]
        public virtual void AjoutMauvaisPattern()
        {
            TechTalk.SpecFlow.ScenarioInfo scenarioInfo = new TechTalk.SpecFlow.ScenarioInfo("Ajout mauvais pattern", new string[] {
                        "Ajout"});
#line 14
this.ScenarioSetup(scenarioInfo);
#line 15
 testRunner.Given("Je vais dans la page d\'ajout");
#line 16
  testRunner.And("Je remplis mal le champs Email");
#line 17
  testRunner.And("je remplis mal les horaires");
#line 18
 testRunner.When("Je clique sur Envoyer");
#line 19
 testRunner.Then("Il doit y avoir des messages d\'erreur");
#line hidden
            testRunner.CollectScenarioErrors();
        }
        
        [NUnit.Framework.TestAttribute()]
        [NUnit.Framework.DescriptionAttribute("Ajout complet")]
        [NUnit.Framework.CategoryAttribute("Ajout")]
        public virtual void AjoutComplet()
        {
            TechTalk.SpecFlow.ScenarioInfo scenarioInfo = new TechTalk.SpecFlow.ScenarioInfo("Ajout complet", new string[] {
                        "Ajout"});
#line 22
this.ScenarioSetup(scenarioInfo);
#line 23
 testRunner.Given("Je vais dans la page d\'ajout");
#line 24
  testRunner.And("Je remplis la partie informations générales");
#line 25
  testRunner.And("Je coche quelques checkbox");
#line 26
  testRunner.And("je remplis Acces");
#line 27
  testRunner.And("Je remplis Horaires");
#line 28
 testRunner.When("Je clique sur Envoyer");
#line 29
 testRunner.Then("Je dois avoir le détail des informations générales");
#line 30
  testRunner.And("Je dois avoir le texte des checkbox");
#line 31
  testRunner.And("Je dois avoir le détail de l\'acces");
#line 32
  testRunner.And("Je dois avoir le détail des horaires");
#line hidden
            testRunner.CollectScenarioErrors();
        }
        
        [NUnit.Framework.TestAttribute()]
        [NUnit.Framework.DescriptionAttribute("Ajout complet 2")]
        [NUnit.Framework.CategoryAttribute("Ajout")]
        public virtual void AjoutComplet2()
        {
            TechTalk.SpecFlow.ScenarioInfo scenarioInfo = new TechTalk.SpecFlow.ScenarioInfo("Ajout complet 2", new string[] {
                        "Ajout"});
#line 35
this.ScenarioSetup(scenarioInfo);
#line 36
 testRunner.Given("Je vais dans la page d\'ajout");
#line 37
  testRunner.And("Je remplis la partie informations générales");
#line 38
  testRunner.And("Je coche quelques checkbox 2");
#line 39
  testRunner.And("je remplis Acces");
#line 40
  testRunner.And("Je remplis Horaires 2");
#line 41
 testRunner.When("Je clique sur Envoyer");
#line 42
 testRunner.Then("Je dois avoir le détail des informations générales");
#line 43
  testRunner.And("Je dois avoir le texte des checkbox 2");
#line 44
  testRunner.And("Je dois avoir le détail de l\'acces");
#line 45
  testRunner.And("Je dois avoir le détail des horaires 2");
#line hidden
            testRunner.CollectScenarioErrors();
        }
        
        [NUnit.Framework.TestAttribute()]
        [NUnit.Framework.DescriptionAttribute("Ajout de lieu deja present")]
        [NUnit.Framework.CategoryAttribute("Ajout")]
        public virtual void AjoutDeLieuDejaPresent()
        {
            TechTalk.SpecFlow.ScenarioInfo scenarioInfo = new TechTalk.SpecFlow.ScenarioInfo("Ajout de lieu deja present", new string[] {
                        "Ajout"});
#line 48
this.ScenarioSetup(scenarioInfo);
#line 49
 testRunner.Given("Je vais dans la page d\'ajout");
#line 50
  testRunner.And("Je remplis lieux deja rentré");
#line 51
 testRunner.When("Je clique sur Envoyer");
#line 52
 testRunner.Then("Il doit y avoir des messages d\'erreur");
#line hidden
            testRunner.CollectScenarioErrors();
        }
        
        [NUnit.Framework.TestAttribute()]
        [NUnit.Framework.DescriptionAttribute("Ajout de lieu et geolocalisation")]
        [NUnit.Framework.CategoryAttribute("Ajout")]
        public virtual void AjoutDeLieuEtGeolocalisation()
        {
            TechTalk.SpecFlow.ScenarioInfo scenarioInfo = new TechTalk.SpecFlow.ScenarioInfo("Ajout de lieu et geolocalisation", new string[] {
                        "Ajout"});
#line 55
this.ScenarioSetup(scenarioInfo);
#line 56
 testRunner.Given("Je vais dans la page d\'ajout");
#line 57
 testRunner.When("Je remplis lieux valide");
#line 58
 testRunner.Then("La position initial doit avoir changée");
#line hidden
            testRunner.CollectScenarioErrors();
        }
        
        [NUnit.Framework.TestAttribute()]
        [NUnit.Framework.DescriptionAttribute("Présence Photo Existante")]
        [NUnit.Framework.CategoryAttribute("Ajout")]
        public virtual void PresencePhotoExistante()
        {
            TechTalk.SpecFlow.ScenarioInfo scenarioInfo = new TechTalk.SpecFlow.ScenarioInfo("Présence Photo Existante", new string[] {
                        "Ajout"});
#line 61
this.ScenarioSetup(scenarioInfo);
#line 62
 testRunner.Given("Je vais dans la page d\'acceuil");
#line 63
 testRunner.And("Je Selectionne la une");
#line 64
 testRunner.When("Je clique sur editer lieu");
#line 65
 testRunner.Then("Je dois avoir une photo presente");
#line hidden
            testRunner.CollectScenarioErrors();
        }
    }
}
#endregion
