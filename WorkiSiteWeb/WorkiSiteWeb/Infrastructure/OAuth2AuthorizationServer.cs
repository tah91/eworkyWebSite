	...
	internal class OAuth2AuthorizationServer : IAuthorizationServer
	{
		...
		public ICryptoKeyStore CryptoKeyStore
		{
			get 
			{
				...
				var store = ModelFactory.GetRepository<INonceRepository>(context);
				return (ICryptoKeyStore)store; 
			}
		}
		...
		public IClientDescription GetClient(string clientIdentifier)
		{
			...
			var consumerRow = acRepo.Get(consumerCandidate => consumerCandidate.ClientIdentifier == clientIdentifier);
			if (consumerRow == null)
			{
		...
		public bool CanBeAutoApproved(EndUserAuthorizationRequest authorizationRequest)
		{
			...
			if (authorizationRequest.ResponseType == EndUserAuthorizationResponseType.AuthorizationCode)
			{
				...
				var requestingClient = acRepo.Get(c => c.ClientIdentifier == authorizationRequest.ClientIdentifier);
		...
		private bool IsAuthorizationValid(HashSet<string> requestedScopes, string clientIdentifier, DateTime issuedUtc, string username)
		{
			...
			var context = ModelFactory.GetUnitOfWork();
			var acaRepo = ModelFactory.GetRepository<ApiClientAuthorizationRepository>(context);

			var grantedScopeStrings = acaRepo.GetMany(auth =>
										  auth.ApiClient.ClientIdentifier == clientIdentifier &&
										  auth.CreatedOnUtc <= issuedUtc &&
										  (!auth.ExpirationDateUtc.HasValue || auth.ExpirationDateUtc.Value >= DateTime.UtcNow) &&
										  auth.Member.Username == username)
										  .Select(auth => auth.Scope);
